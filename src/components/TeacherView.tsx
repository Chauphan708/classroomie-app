...
const SOUNDS = {
  ...
  buzzer: 'https://assets.mixkit.co/active_storage/sfx/1435/1435-preview.mp3', // <-- Tiếng kèn Fanfare to, rõ
};
...
export const TeacherView: React.FC<Props> = ({ store, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // Kích hoạt âm thanh khi có người thắng MỚI
      ...
      <div className="flex-1 flex overflow-hidden relative">
          ...
          {showActivityLog && (
            <div className="w-96 bg-white border-l border-slate-200 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 absolute md:relative right-0 h-full z-30">
                <div className="p-4 border-b border-slate-100 bg-white"><div className="flex justify-between items-center mb-4"><h3 className="font-bold text-slate-800 flex items-center gap-2"><MessageSquare size={18} className="text-indigo-600"/> Tường Lớp</h3><button onClick={() => setShowActivityLog(false)} className="md:hidden text-slate-400"><X size={20}/></button></div><div className="flex gap-1 mb-3 bg-slate-100 p-1 rounded-xl">{['all', 'help', 'finished', 'raised'].map((t) => ( <button key={t} onClick={() => setFilterStatus(t as any)} className={`flex-1 text-[10px] py-1.5 rounded-lg font-bold uppercase transition-all ${filterStatus === t ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{t === 'all' ? 'Tất cả' : t === 'help' ? 'Cần giúp' : t === 'finished' ? 'Đã xong' : 'Giơ tay'}</button> ))}</div><div className="flex gap-2 mb-3"><select value={filterStudentId} onChange={(e) => setFilterStudentId(e.target.value)} className="text-xs border-2 border-slate-200 bg-slate-50 rounded-lg p-2 flex-1 outline-none focus:border-indigo-500 font-medium text-slate-600"><option value="all">-- Lọc theo học sinh --</option>{students.map(s => <option key={s.id} value={s.id}>{s.name} (Tổ {s.group})</option>)}</select></div><div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-100"><button onClick={() => updateWallConfig({ isPublic: !wallConfig.isPublic })} className={`p-2 rounded-xl flex flex-col items-center gap-1 border-2 transition-all font-bold ${wallConfig.isPublic ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300'}`}>{wallConfig.isPublic ? <Globe size={20} /> : <Lock size={20} />}{wallConfig.isPublic ? 'Công khai' : 'Riêng tư'}</button><button disabled={!wallConfig.isPublic} onClick={() => updateWallConfig({ showNames: !wallConfig.showNames })} className={`p-2 rounded-xl flex flex-col items-center gap-1 border-2 transition-all font-bold ${!wallConfig.isPublic ? 'opacity-50 cursor-not-allowed bg-slate-50 text-slate-300 border-slate-100' : wallConfig.showNames ? 'bg-green-50 border-green-200 text-green-600' : 'bg-purple-50 border-purple-200 text-purple-600'}`}>{wallConfig.showNames ? <Eye size={20} /> : <EyeOff size={20} />}{wallConfig.showNames ? 'Hiện tên' : 'Ẩn tên HS'}</button></div></div>
