...
import { 
  ...
  Trash2, MessageSquare, Send, Sparkles, X, Loader2, LogOut, Clock, Globe, Eye, EyeOff, Lock, Unlock
} from 'lucide-react';
...
const SOUNDS = {
  ...
  // Âm thanh chuông reo (Ding Ding)
  buzzer: 'https://assets.mixkit.co/active_storage/sfx/2000/2000-preview.mp3', 
};

export const TeacherView: React.FC<Props> = ({ store, onLogout }) => {
  const { state, updateWallConfig, resetBuzzer, lockBuzzer, resetAllStatuses, removeStudent } = store;
  ...
  const prevWinnerRef = useRef<string | null>(null); // Theo dõi người thắng cũ
  ...
  useEffect(() => {
    ...
    // Check CHUÔNG REO (Mới)
    if (state.buzzerWinnerId && state.buzzerWinnerId !== prevWinnerRef.current) {
        playSound('buzzer'); // Reo chuông bên máy Giáo Viên
    }

    prevStudentsRef.current = { ...state.students };
    prevMessagesLengthRef.current = messages.length;
    prevWinnerRef.current = state.buzzerWinnerId;
  }, [state.students, messages, state.buzzerWinnerId]);

  // ... (Phần logic AI và formatTime giữ nguyên, tôi chỉ update giao diện bên dưới) ...
  ...
  const getCardColor = (s: StudentStatus) => {
    if (state.buzzerWinnerId === s.id) return 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-300 transform scale-105 ring-4 ring-yellow-400';
    ...
    <div className="h-screen bg-slate-50 flex flex-col md:flex-row overflow-hidden font-sans">
      <aside className="bg-white w-full md:w-72 flex-shrink-0 border-r border-slate-200 shadow-sm flex flex-col z-20">
        ...
        <nav className="p-4 space-y-3 flex-1 overflow-y-auto">
          <div className="grid grid-cols-3 gap-2 mb-4">
             {/* Stats Counters (Giữ nguyên) */}
             <div className="bg-red-50 p-2 rounded-xl border border-red-100 flex flex-col items-center justify-center"><span className="font-black text-xl text-red-600">{students.filter(s => s.needsHelp).length}</span><span className="text-[10px] uppercase font-bold text-red-400">Cần giúp</span></div>
             <div className="bg-green-50 p-2 rounded-xl border border-green-100 flex flex-col items-center justify-center"><span className="font-black text-xl text-green-600">{students.filter(s => s.isFinished).length}</span><span className="text-[10px] uppercase font-bold text-green-400">Xong bài</span></div>
             <div className="bg-yellow-50 p-2 rounded-xl border border-yellow-100 flex flex-col items-center justify-center"><span className="font-black text-xl text-yellow-600">{students.filter(s => s.handRaised).length}</span><span className="text-[10px] uppercase font-bold text-yellow-500">Giơ tay</span></div>
          </div>

          <div className="space-y-2">
             <p className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-2">Công cụ</p>
             
             {/* NÚT CHUÔNG ĐÃ SỬA LOGIC */}
             {state.buzzerActive ? (
                <button onClick={lockBuzzer} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-white bg-red-500 hover:bg-red-600 rounded-xl transition-all shadow-md animate-pulse">
                    <Lock size={18} /> Khóa Chuông
                </button>
             ) : (
                <button onClick={resetBuzzer} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 rounded-xl transition-all shadow-sm">
                    <Unlock size={18} /> Mở Chuông Mới
                </button>
             )}
             ...
             <button onClick={resetAllStatuses} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-600 bg-white border border-slate-200 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all shadow-sm">
              ...
              <button onClick={() => setShowActivityLog(!showActivityLog)} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl transition-all shadow-sm border ${showActivityLog ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                <MessageSquare size={18} /> Tường lớp {messages.length > 0 && <span className="ml-auto bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{messages.length}</span>}
             </button>
          </div>
        </nav>
      ...
      <div className="flex-1 flex overflow-hidden relative">
          <main className="flex-1 p-4 md:p-8 overflow-y-auto bg-slate-50">
            {state.buzzerWinnerId && (
                <div className="mb-8 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-6 text-white shadow-xl shadow-indigo-200 flex items-center justify-between animate-in zoom-in duration-300">
                    <div className="flex items-center gap-6">
                        <div className="bg-white/20 p-4 rounded-full backdrop-blur-sm"><Bell className="w-10 h-10 animate-bounce" /></div>
                        <div><p className="text-indigo-100 text-sm font-bold uppercase tracking-wide opacity-80">Người nhanh nhất</p><h2 className="text-4xl font-black tracking-tight">{state.students[state.buzzerWinnerId]?.name || 'Unknown'}</h2></div>
                    </div>
                    {/* Nút reset nhanh ngay trên banner */}
                    <button onClick={resetBuzzer} className="bg-white text-indigo-600 px-6 py-2 rounded-xl font-bold shadow-lg hover:bg-indigo-50 transition-colors">Mở Vòng Mới</button>
                </div>
            )}
                    ...
                    <div key={student.id} className={`relative p-4 rounded-2xl transition-all duration-300 flex flex-col items-center gap-3 group ${getCardColor(student)}`}>
                        ...
                        <div className="text-center w-full">
                            ...
                            <div className="text-xs font-medium mt-1 min-h-[20px]">
                                {student.needsHelp ? <span className="text-red-500 bg-red-50 px-2 py-0.5 rounded-full">Cần giúp {formatTime(student.needsHelpAt)}</span> : student.isFinished ? <span className="text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Xong {formatTime(student.isFinishedAt)}</span> : student.handRaised ? <span className="text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-full">Giơ tay {formatTime(student.handRaisedAt)}</span> : ''}
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {students.length === 0 && (<div className="flex flex-col items-center justify-center h-[60vh] text-slate-300"><Users className="w-24 h-24 mb-4 opacity-50" strokeWidth={1} /><p className="text-xl font-medium">Đang chờ học sinh tham gia...</p><p className="text-sm">Hãy chia sẻ mã lớp học cho học sinh</p></div>)}
          </main>

          {/* Phần Chat/Wall bên phải (Giữ nguyên giao diện đẹp từ bước trước) */}
          {showActivityLog && (
            <div className="w-96 bg-white border-l border-slate-200 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 absolute md:relative right-0 h-full z-30">
                <div className="p-4 border-b border-slate-100 bg-white">
                    <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-slate-800 flex items-center gap-2"><MessageSquare size={18} className="text-indigo-600"/> Tường Lớp</h3><button onClick={() => setShowActivityLog(false)} className="md:hidden text-slate-400"><X size={20}/></button></div>
                    <div className="flex gap-1 mb-3 bg-slate-100 p-1 rounded-xl">
                         {['all', 'help', 'finished', 'raised'].map((t) => ( <button key={t} onClick={() => setFilterStatus(t as any)} className={`flex-1 text-[10px] py-1.5 rounded-lg font-bold uppercase transition-all ${filterStatus === t ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{t === 'all' ? 'Tất cả' : t === 'help' ? 'Cần giúp' : t === 'finished' ? 'Đã xong' : 'Giơ tay'}</button> ))}
                    </div>
                    <div className="flex gap-2 mb-3"><select value={filterStudentId} onChange={(e) => setFilterStudentId(e.target.value)} className="text-xs border-2 border-slate-200 bg-slate-50 rounded-lg p-2 flex-1 outline-none focus:border-indigo-500 font-medium text-slate-600"><option value="all">-- Lọc theo học sinh --</option>{students.map(s => <option key={s.id} value={s.id}>{s.name} (Tổ {s.group})</option>)}</select></div>
                    <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-100">
                        ...
                        <button disabled={!wallConfig.isPublic} onClick={() => updateWallConfig({ showNames: !wallConfig.showNames })} className={`p-2 rounded-xl flex flex-col items-center gap-1 border-2 transition-all font-bold ${!wallConfig.isPublic ? 'opacity-50 cursor-not-allowed bg-slate-50 text-slate-300 border-slate-100' : wallConfig.showNames ? 'bg-green-50 border-green-200 text-green-600' : 'bg-purple-50 border-purple-200 text-purple-600'}`}>{wallConfig.showNames ? <Eye size={20} /> : <EyeOff size={20} />}{wallConfig.showNames ? 'Hiện tên' : 'Ẩn tên HS'}</button>
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                    {!wallConfig.isPublic ? ( <div className="flex flex-col items-center justify-center h-full text-slate-400 space-y-4 p-6 text-center"><div className="bg-white p-6 rounded-full shadow-sm border border-slate-100"><Lock size={40} className="text-slate-300" /></div><div><p className="font-bold text-slate-600 text-lg">Chế độ Riêng tư</p><p className="text-sm mt-1 leading-relaxed">Tin nhắn đang bị ẩn trên màn hình chung.</p><div className="mt-4 text-xs font-bold bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full inline-block">{messages.length} tin nhắn trong lớp</div></div></div> ) : ( <> {filteredMessages.length === 0 && <p className="text-center text-slate-400 text-sm font-medium py-10">Chưa có tin nhắn nào</p>} {filteredMessages.map((msg) => ( <div key={msg.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-2"> <div className="flex justify-between items-start mb-2"> <div className="flex items-center gap-2"> <span className={`font-bold text-sm ${wallConfig.showNames ? 'text-indigo-600' : 'text-purple-600 bg-purple-50 px-2 py-0.5 rounded-md'}`}>{wallConfig.showNames ? msg.senderName : 'Bạn giấu tên'}</span> {!wallConfig.showNames && <EyeOff size={12} className="text-purple-300"/>} </div> <span className="text-[10px] text-slate-400 font-medium">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span> </div> {msg.text && <p className="text-slate-700 text-sm leading-relaxed">{msg.text}</p>} {msg.imageUrl && ( <div className="mt-2 rounded-xl overflow-hidden border border-slate-200"><img src={msg.imageUrl} alt="upload" className="w-full h-auto object-cover" /></div> )} </div> ))} <div ref={messagesEndRef} /> </> )}
                </div>
            </div>
          )}
      </div>
