...
export const StudentView: React.FC<Props> = ({ session, store, onLogout }) => {
  ...
  // State quản lý việc mình đã bấm hay chưa
  ...
  // Logic khóa: Khóa khi (GV chưa mở) HOẶC (Đã có người thắng mà ko phải mình)
  const isBuzzerLocked = !state.buzzerActive || (state.buzzerWinnerId !== null && !isBuzzerWinner);

  // --- SỬA LỖI QUAN TRỌNG TẠI ĐÂY ---
  // Reset nút bấm ngay khi:
  // 1. Giáo viên Reset (buzzerWinnerId = null)
  // 2. Hoặc Giáo viên Mở lại chuông (buzzerActive = true)
  useEffect(() => {
    if (state.buzzerWinnerId === null || state.buzzerActive) {
       setBuzzerPressed(false); // Cho phép bấm lại
    }
  }, [state.buzzerWinnerId, state.buzzerActive]);

  // Hiệu ứng pháo giấy khi thắng
  useEffect(() => {
    if (isBuzzerWinner) {
       const fire = (confetti as any).default || confetti;
       fire({ particleCount: 200, spread: 100, origin: { y: 0.6 }, ticks: 300 });
    }
  }, [isBuzzerWinner]); // Tách riêng effect này để không bị conflict
  ...
  const pressBuzzer = () => {
    // Chỉ cho bấm nếu chưa khóa và CHƯA BẤM lần nào trong lượt này
    if (!isBuzzerLocked && !buzzerPressed) {
      setBuzzerPressed(true); // Khóa nút ngay lập tức để tránh spam
      ...
      <header className="bg-white px-4 py-3 shadow-sm border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-md mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
                 <img src={`https://picsum.photos/seed/${myStatus.avatarSeed}/50/50`} alt="avatar" className="w-10 h-10 rounded-full border-2 border-indigo-500 shadow-sm" />
                 <div><h2 className="font-bold text-slate-800 leading-tight text-lg">{session.name}</h2><p className="text-xs font-bold text-indigo-500 uppercase tracking-wide">Tổ {myStatus.group}</p></div>
            </div>
            <button onClick={onLogout} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors"><LogOut size={24} /></button>
        </div>
      </header>
      ...
      <main className="flex-1 p-4 max-w-md mx-auto w-full space-y-4 pb-6">
        <div className={`p-1 rounded-[2.5rem] bg-white shadow-xl transition-all duration-300 ${isBuzzerWinner ? 'scale-105 ring-4 ring-yellow-400' : ''}`}>
            <div className={`rounded-[2.2rem] p-6 text-center transition-all duration-500 ${isBuzzerWinner ? 'bg-gradient-to-br from-indigo-500 to-purple-600' : 'bg-slate-50'}`}>
                ...
                <button
                    onClick={pressBuzzer}
                    // Disable khi: Khóa server HOẶC Đã bấm rồi
                    ...
                    className={`w-full aspect-square rounded-full flex flex-col items-center justify-center transition-all duration-200 relative overflow-hidden group
                        ${isBuzzerWinner 
                            ...
                            : !state.buzzerActive
                                ...
                                : buzzerPressed 
                                    ? 'bg-indigo-400 text-white cursor-wait' // Đã bấm, chờ kết quả
                ...
                >
                    ...
                    {!state.buzzerActive && !state.buzzerWinnerId && (<div className="absolute inset-0 flex items-center justify-center bg-black/5"><Lock className="opacity-20 w-16 h-16" /></div>)}
                </button>
            </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden flex flex-col h-[450px]">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center"><h3 className="font-bold text-indigo-600 flex items-center gap-2"><span className="bg-indigo-100 p-1.5 rounded-lg"><Send size={16}/></span> Tường Lớp</h3><span className={`text-[10px] font-bold px-3 py-1 rounded-full border ${wallConfig.isPublic ? 'bg-green-100 text-green-700 border-green-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>{wallConfig.isPublic ? (wallConfig.showNames ? 'CÔNG KHAI' : 'ẨN DANH') : 'RIÊNG TƯ'}</span></div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
             ...
             {visibleMessages.map(msg => { const isMe = msg.senderId === session.id; const showName = isMe || wallConfig.showNames; return ( <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}> <div className={`max-w-[85%] rounded-2xl p-3 shadow-sm ${isMe ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white rounded-br-none' : 'bg-white border border-slate-100 rounded-bl-none text-slate-700'}`}> <div className={`text-[10px] font-bold mb-1 ${isMe ? 'text-indigo-200' : 'text-slate-400'} flex items-center gap-1 uppercase tracking-wide`}> {showName ? msg.senderName : 'Bạn bí ẩn'} {!showName && <EyeOff size={10} />} </div> {msg.imageUrl && ( <img src={msg.imageUrl} alt="post" className="rounded-lg mb-2 max-h-40 object-cover w-full border border-black/10" /> )} {msg.text && <p className="text-sm font-medium">{msg.text}</p>} </div> </div> ); })}
